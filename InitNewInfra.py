...
for datacenter in config:
    ...
    for n in nodes:
        ...
        if n['Server'] == 'vault':
            ...
            node.ExecCommand("sudo rm -rf /etc/vault.d/*", True)
            ...
            print("Sleeping for 5 seconds until Vault Server %s is ready...\n" % n['hostname'])
            time.sleep(5)
    print("")
    for n in nodes:
        node = n['node_client']
    ...
    if datacenter['pki_engine'] == 'vault':
        print("Configuring Connect to use Vault as PKI Engine for TLS certificates...\n")
    else:
